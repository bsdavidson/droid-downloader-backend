...
class RedisFilesTransform extends Transform {
  ...
  async _transform(chunk, encoding, callback) {
    try {
      ...
      console.warn(
        `worker.RedisFilesTransform: error (${chunk.filePath}): ${err}`
      );
    }
    callback();
  }
...
class RedisFileIDsTransform extends Transform {
  ...
  async _transform(chunk, encoding, callback) {
    try {
      ...
      console.warn(
        `worker.RedisFileIDsTransform: error (${chunk.filePath}): ${err}`
      );
    }
    callback();
  }
...
class RedisFileGeosWritable extends Writable {
  ...
  async _write(chunk, encoding, callback) {
    try {
      ...
      console.warn(
        `worker.RedisFileGeosWritable: error (${chunk.filePath}): ${err}`
      );
    }
    callback();
  }
...
class ThumbnailWritable extends Writable {
  constructor(options) {
    ...
    this.thumbnailsPath = path.join(os.tmpdir(), "thumbs");
    ...
    console.log(
      `worker.ThumbnailWritable: Thumbnail Path: ${this.thumbnailsPath}`
    );
  }
    ...
    try {
      ...
      sharp(chunk.filePath)
        ...
        .toFile(thumbnailImagePath, function(err) {
          if (err) {
            console.warn(
              `worker.ThumbnailWritable: error (${chunk.filePath}): ${err}`
            );
          }
          callback();
        });
...
function monitorPath(path) {
  ...
  const thumbnailWritable = new ThumbnailWritable();
